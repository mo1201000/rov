...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        self.canvas=tk.Canvas( window, width=900,height=480,bg='white')
        self.canvas.pack()

        # Button that lets the user take a snapshot
        self.btn_snapshot=tk.Button(window, text="SCEERNSHOT", command=self.snapshot, width='15',height='4',bg='orange',fg='black')
        self.btn_snapshot.pack(side=tk.LEFT)

        #video control buttons

        self.btn_start=tk.Button(window, text='START', command=self.open_camera, width='15',height='4',bg='orange',fg='black')
        self.btn_start.pack(side=tk.LEFT)

        self.btn_Reset=tk.Button(window, text='RESET', command= self.restart_camera,width='15',height='4',bg='orange',fg='black')
        self.btn_Reset.pack(side=tk.LEFT)

        self.btn_stop=tk.Button(window, text='STOP', command=self.close_camera,width='15',height='4',bg='orange',fg='black')
        self.btn_stop.pack(side=tk.LEFT)
        
        # quit button
        self.btn_quit=tk.Button(window, text='QUIT', command=quit,width='16',height='4',bg='orange',fg='black')
        self.btn_quit.pack(side=tk.RIGHT)

        self.btn_label1=tk.Label(window,text="",width='8',height='1',background='green',fg='black')
        self.btn_label1.place(x=800, y=50)

        self.btn_label2=tk.Label(window, text="" ,width='8',height='1',bg='green',fg='black')
        self.btn_label2.place(x=800, y=90)

        self.btn_label2=tk.Label(window, text="" ,width='8',height='1',bg='green',fg='black')
        self.btn_label2.place(x=800, y=130)

        self.btn_label3=tk.Label(window,text="",width='8',height='1',background='green',fg='black')
        self.btn_label3.place(x=800, y=170)

        self.btn_label4=tk.Label(window, text="" ,width='8',height='1',bg='green',fg='black')
        self.btn_label4.place(x=800, y=200)

        #self.btn_label3=tk.Button(window, command=self.restart_camera,width='8',height='2',bg='black',fg='black')

        #self.btn_label3.place(x=800,y=150)
    ...
    def snapshot(self):
        ...
        if ret:
            cv2.imwrite("frame-"+time.strftime("%d-%m-%Y-%H-%M-%S")+".jpg",cv2.cvtColor(frame,cv2.Format_RGB888))
...
class ElapsedTimeClock:
    def __init__(self,window):
        self.T=tk.Label(window,text='00:00:00',font=('times', 25, 'bold'), bg='green',fg='red')
